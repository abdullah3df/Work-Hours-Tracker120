...
const ReportModal: React.FC<ReportModalProps> = ({ isOpen, onClose, logs, profile, t, language, user }) => {
  ...
  const [filterTypes, setFilterTypes] = useState<LogType[]>(['work', 'sickLeave', 'vacation']);
  ...
  const filteredLogs = useMemo(() => {
    ...
    return [...logs]
      ...
      .filter(log => filterTypes.includes(log.type))
      .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  }, [logs, startDate, endDate, filterTypes]);
  ...
  const captureReport = async (type: 'pdf' | 'image') => {
    const input = reportContentRef.current;
    if (!input) return;

    const scrollableContainer = input.parentElement as HTMLElement;
    if (!scrollableContainer) return;

    const originalStyles = {
      overflow: scrollableContainer.style.overflow,
      height: scrollableContainer.style.height,
      scrollTop: scrollableContainer.scrollTop,
      inputWidth: input.style.width,
    };

    // Make entire content visible for capture
    scrollableContainer.scrollTop = 0;
    scrollableContainer.style.overflow = 'visible';
    scrollableContainer.style.height = 'auto';

    // Set a fixed desktop width to ensure a consistent, full-featured layout in the PDF.
    const captureWidth = 840; // A good desktop width
    ...
    try {
      const canvas = await html2canvas(input, {
        ...
        width: captureWidth, // Use the same fixed width for the canvas
        windowWidth: captureWidth, // This hints html2canvas to use this width for media queries
      });
      ...
      if (type === 'image') {
        ...
        const margin = 10;
        const pdfUsableWidth = pdf.internal.pageSize.getWidth() - margin * 2;
        const pdfUsableHeight = pdf.internal.pageSize.getHeight() - margin * 2;
        
        const canvasWidth = canvas.width;
        const canvasHeight = canvas.height;
        const ratio = canvasHeight / canvasWidth;

        const imgWidth = pdfUsableWidth;
        const imgHeight = pdfUsableWidth * ratio;
        
        let heightLeft = imgHeight;
        let position = 0;

        pdf.addImage(imgData, 'PNG', margin, margin, imgWidth, imgHeight);
        heightLeft -= pdfUsableHeight;

        while (heightLeft > 0) {
          position -= pdfUsableHeight;
          pdf.addPage();
      ...
      scrollableContainer.style.overflow = originalStyles.overflow;
      scrollableContainer.style.height = originalStyles.height;
      scrollableContainer.scrollTop = originalStyles.scrollTop;
      input.style.width = originalStyles.inputWidth;
    }
  };
  ...
  const logTypeOptions: { key: LogType, label: string }[] = [
    { key: 'work', label: t('work') },
    { key: 'sickLeave', label: t('sickLeave') },
    { key: 'vacation', label: t('vacation') },
  ];

  const handleFilterTypeChange = (type: LogType) => {
    setFilterTypes(prev =>
      prev.includes(type)
  ...
  return (
    <div className="fixed inset-0 bg-black/75 flex justify-center items-center z-50 p-4 transition-opacity duration-300">
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[90vh]">
                ...
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                        {logTypeOptions.map(opt => (
                            <label key={opt.key} className="flex items-center space-x-2 rtl:space-x-reverse cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={filterTypes.includes(opt.key)}
                                    onChange={() => handleFilterTypeChange(opt.key)}
