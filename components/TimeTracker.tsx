...
const TimeTracker: React.FC<TimeTrackerProps> = ({ logs, addLog, profile, t, showToast, language }) => {
  ...
  const remainingVacationDays = useMemo(() => {
    if (!profile.totalVacationDaysPerYear && profile.totalVacationDaysPerYear !== 0) {
      return '—';
    }
    const currentYear = new Date().getFullYear();
    const usedVacationDays = logs.filter(log =>
      log.type === 'vacation' && new Date(log.date).getFullYear() === currentYear
    ).length;
    return profile.totalVacationDaysPerYear - usedVacationDays;
  }, [logs, profile.totalVacationDaysPerYear]);
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 sm:p-8 rounded-2xl shadow-2xl shadow-shadow-color-light dark:shadow-shadow-color-dark w-full max-w-4xl mx-auto transition-all duration-300">
      ...
      <div className="flex flex-col sm:flex-row justify-around items-center mb-8 text-center space-y-6 sm:space-y-0">
        ...
        <div className="w-px h-16 bg-gray-300/50 dark:bg-gray-600/50 hidden sm:block"></div>
        <div>
          <p className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('remainingVacation')}</p>
          <p className="text-4xl sm:text-5xl md:text-6xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text text-transparent font-mono">{remainingVacationDays}</p>
        </div>
      </div>
